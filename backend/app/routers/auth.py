...
def get_avatar_upload_url(
    ...
    # Return the s3_key so the frontend can store it instead of the public URL
...
def update_avatar_url(
    ...
    # Store the s3_key (not the public URL) so we can generate pre-signed URLs
...
def get_avatar_url(current_user=Depends(get_current_user)):
    """Return a fresh pre-signed GET URL for the current user's avatar."""
    if not current_user.profile_picture_url:
        return {"url": None}
    key = current_user.profile_picture_url
    # If it's already an s3_key (starts with profiles/), generate presigned URL
    # If it's a full URL (legacy), extract the key
    if key.startswith("http"):
        # Legacy full URL — extract the key after the bucket domain
        import re
    ...
    try:
        ...
        url = generate_presigned_get(key, expires=3600)  # 1 hour
        return {"url": url}
    except Exception:
        return {"url": None}
